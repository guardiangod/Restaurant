...
<c:url var="searchURL" value="/menu/rest/result" />
<form:form modelAttribute="restaurantSearchTo" action="${searchURL}" method="post" id="srchMenuForm" name="srchMenuForm" >
	<p id="srchMenuError" class="ui-state-error ui-corner-all">
		<span class="ui-icon ui-icon-alert"></span>
		<span>Search for restaurants in your neighbourhood to view the menu</span>
	</p>
	<table border="0" summary="search" width="100%" cellspacing="10px">
		<tr>
			<td>Which Restaurant?</td>
			<td>What Cuisine?</td>
			<td colspan="2">Near (Address, City, Location, or Postcode)</td>
		</tr>
		<tr>
			<td width="30%">
	...
	<c:if test="${!empty restaurantList}">
		...
		<c:forEach items="${restaurantList}" var="rs">
			...
			<tr>
				<td width="15%"><img src="../images/${rs.restaurantImage}" alt="${rs.restaurantImage}" width="74px" /></td>
				...
				<td width="20%">
					<button class="viewMenuBtn" name="viewMenuBtn${rs.restaurantId}" id="viewMenuBtn${rs.restaurantId}">
						View Menu
					</button>
				</td>
			</tr>
